...
interface FormCarroProps {
    carros: Carro[],
    setCarros: (setCarros: Carro[]) => void,
    marca: string,
...
export const FormCarro = ({ carros, setCarros, marca, setMarca, modelo, setModelo, cor, setCor, placa, setPlaca, preco, setPreco, quilometragem,
    setQuilometragem, dataFabricacao, setDataFabricacao, isEmEdicao, idEmEdicao, limparForm }: FormCarroProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const dadosCarro: Omit<Carro, 'id'> = {
            marca,
            modelo,
            cor,
            placa,
        ...
        if (isEmEdicao) {
            axios.put<Carro>(`${API_URL}/carros/${idEmEdicao}`, dadosCarro)
                .then(res => {
                    alert("Carro atualizado com sucesso!!!");
                    setCarros(carros.map(carro => carro.id === idEmEdicao ? res.data : carro))
                    limparForm()
                }).catch(err => {
                    console.error("Erro na resposta da API ao editar: " + err);
                    alert("Erro ao atualizar os dados do carro.");
                });

        } else {
            try {
                axios.post(API_URL + '/carros', dadosCarro)
                    .then(res => {
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl w-full h-auto flex flex-col gap-6 bg-white p-8 rounded-2xl shadow-md" >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    {isEmEdicao && (
                        <button
                            className="mt-2 px-81 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition duration-200"
                            onClick={limparForm}
                        >
                            Cancelar
                        </button>
